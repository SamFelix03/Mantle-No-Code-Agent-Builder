...
// TokenFactory Contract Address (Mantle Sepolia Testnet)
const FACTORY_ADDRESS = '0x1E491de1a08843079AAb4cFA516C717597344e50';
...
// NFTFactory Contract Address (Mantle Sepolia Testnet)
const NFT_FACTORY_ADDRESS = '0x0866f40D55E96b2D74995203Caff032aD81c14B0';
...
// DAOFactory Contract Address (Mantle Sepolia Testnet)
const DAO_FACTORY_ADDRESS = '0x3ea6d1c84481f89aac255a7abc375fe761653cda';
...
// Airdrop Contract Address (Mantle Sepolia Testnet)
const AIRDROP_CONTRACT_ADDRESS = '0x700D3D55ec6FC21394A43b02496F320E02873114';

// YieldCalculator Contract Address (Mantle Sepolia Testnet)
const YIELD_CALCULATOR_ADDRESS = '0x6034FAcE419117Af850411CA58d83c1405F3398F';
  ...
  try {
    ...
    if (tokenAddress) {
      ...
      return res.json({
        ...
        explorerUrl: `https://sepolia.mantlescan.xyz/tx/${receipt.hash}`
      });
    }

    // Native token transfer (original behavior)
    console.log('Transferring native token (MNT)');
  ...
  try {
    ...
    try {
      ...
      if (gasPrice && gasPrice > 0n) {
        ...
        console.log('Estimated transaction cost:', ethers.formatEther(estimatedCost), 'MNT');
        ...
        if (balance < estimatedCost) {
          ...
          console.warn('   Balance:', ethers.formatEther(balance), 'MNT');
          console.warn('   Estimated cost:', ethers.formatEther(estimatedCost), 'MNT');
...
app.post('/create-nft-collection', async (req, res) => {
  try {
    ...
    return res.json({
      ...
      explorerUrls: {
        collection: `https://sepolia.mantlescan.xyz/tx/${createReceipt.hash}`,
        mint: `https://sepolia.mantlescan.xyz/tx/${mintReceipt.hash}`
      },
      nextSteps: [
...
app.post('/create-and-mint-nft', async (req, res) => {
  try {
    ...
    return res.json({
      ...
      explorerUrl: `https://sepolia.mantlescan.xyz/tx/${mintReceipt.hash}`,
...
app.post('/swap', async (req, res) => {
  try {
    ...
    // Uniswap V3 SwapRouter02 address on Mantle
    // Note: This is the mainnet address. For testnet, verify if Uniswap is deployed on Mantle Sepolia
    const SWAP_ROUTER_ADDRESS = '0x738fD6d10bCc05c230388B4027CAd37f82fe2AF2';
...
// Airdrop endpoint - batch transfer native MNT tokens to multiple addresses
...
app.post('/api/balance/erc20', async (req, res) => {
  try {
    ...
    // Get Zerion API authorization from environment (or use default)
    const zerionAuth = process.env.ZERION_API_AUTH || 'Basic Ok1hcnNoYWwyMDA0JA==';
  ...
  try {
    ...
    return res.json({
      ...
      approvalTransaction: approvalTxHash ? {
        hash: approvalTxHash,
        explorerUrl: `https://sepolia.mantlescan.xyz/tx/${approvalTxHash}`
      } : null,
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', network: 'Mantle Sepolia Testnet' });
});
